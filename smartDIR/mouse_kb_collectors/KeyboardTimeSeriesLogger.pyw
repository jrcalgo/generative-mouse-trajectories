...
class CollectKeyboardData:
    def __init__(self, kb_layout="QWERTY"):
        ...
        self.TYPES: dict = {'timestamp': float, 'key': str, 'state': str, 'press_duration': float,
                            'key_press_delay': float, 'special_key': bool, 'wpm': float}
    ...
    def collect_data(self, activation: bool = True):
        ...
        def _on_key_press(key: kb.Key):
            ...
            self.new_press_row = pd.DataFrame([{'timestamp': timestamp, 'key': key, 'state': state,
                                                'press_duration': press_duration, 'key_press_delay': key_press_delay,
                                                'special_key': special_key, 'wpm': wpm}])
        ...
        def _on_key_release(key: kb.Key):
            ...
            self.new_release_row = pd.DataFrame([{'timestamp': timestamp, 'key': key, 'state': state,
                                                  'press_duration': press_duration, 'key_press_delay': key_press_delay,
            ...
            with open(file, file_exists['mode'], newline='\n' if file_exists['existence'] else '') as file:
                ...
                for key in self.data['key']:
                    if len(key) == 1:
                        ...
                        writer.writerow(
                            np.array2string(np.array(new_row), separator='').replace('[', '').replace(']', ''))
                        new_row = []
...
if __name__ == '__main__':
    keyboard_data = CollectKeyboardData()
